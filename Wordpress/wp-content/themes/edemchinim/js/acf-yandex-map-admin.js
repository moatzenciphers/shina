/* Patched ACF: Yandex Maps Field 1.1 admin script: supports multiple markers. */
(function ($) {
    ...
    function initialize_field($el) {
        ...
        function create_mark(coords, type, size, id, content, event) {
            ...
            if (id == undefined) {
                var mark_ids = $params.marks.map(function (mark) {
                    return parseInt(mark.id, 10) || 0;
                });
                mark_id = mark_ids.length ? Math.max.apply(null, mark_ids) + 1 : 1;
            }
            ...
            place_mark.events.add('click', function (event) {
        ...
        function show_mark_editor(mark) {
            ...
            var html = '<div class="ya-editor" style="margin: 5px"><form name="mark"><input type="hidden" name="id" value="' +
                mark.properties.get('id') + '"><textarea name="content" rows="5" cols="40">' + escape_html(mark.properties.get('content')) +
